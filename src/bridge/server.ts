// Bridge server — runs inside the MCP host process.
//
// Opens a local socket (Unix domain socket on POSIX, loopback TCP on
// Windows). The bundled CLI in code-api mode connects to it and
// forwards each call as a newline-delimited JSON request. The server
// resolves the operation against the manifest, executes via the
// supplied Client (and optional ExecuteFn hook), and sandboxes the
// raw response — returning a `SandboxResult` whose `ref` points at
// the full JSON on disk and whose `summary` carries the trim
// projection.
//
// Generalized from jira-mcp/src/codeapi/bridge.ts. No jira specifics
// remain: client, manifest, trim registry, sandbox instance, and
// socket env-var name are all parameters.
...
// representation suitable for setting in the host's socket env var.
// The string is what the client uses to reconnect; the listen options
...
// return EINVAL if we try to bind a longer path. Session cache paths
// under tmpdir can edge over that on macOS for long session IDs, so
// we keep a short alternative under `${tmpdir}` keyed by a hash of
// the full session path.
const UNIX_SOCKET_PATH_MAX = 100;

export interface DefaultBridgeAddressOpts {
  // Absolute path of the session cache directory under which the
  // socket file is preferred. Comes from the sandbox instance.
  sessionCacheDir: string;
  // Prefix for the fallback short socket path under tmpdir.
  // Use a short slug, e.g. "jmcp", "bmcp", "cmcp" (4 chars + hash =
  // well under any sun_path limit). Defaults to "mcp".
  socketPrefix?: string;
}

function shortSocketPath(fullCachePath: string, prefix: string): string {
  const hash = createHash("sha256")
    .update(fullCachePath)
    .digest("hex")
    .slice(0, 12);
  return path.join(os.tmpdir(), `${prefix}-${hash}.sock`);
}

export function defaultBridgeAddress(
  opts: DefaultBridgeAddressOpts,
): BridgeAddress {
  ...
  const preferred = path.join(opts.sessionCacheDir, "ipc.sock");
  const prefix = opts.socketPrefix ?? "mcp";
  const sockPath =
    Buffer.byteLength(preferred, "utf8") <= UNIX_SOCKET_PATH_MAX
      ? preferred
      : shortSocketPath(preferred, prefix);
  return {
    display: sockPath,
...
export interface StartBridgeOpts {
  ...
  // The trim projection registry. The bridge looks up `op.trim` keys
  // here to derive the in-band `summary` field on each SandboxResult.
  // Servers without trim projections can pass `{}`.
  trimRegistry: TrimRegistry;
  address?: BridgeAddress;
  // The user's blocklist of operations. Forwarded to invokeOperationRaw
  // for every bridge call so disabling propagates from classic mode to
  // code-api mode uniformly.
  disabledActions?: readonly string[];
  // Optional per-server execution hook (multi-API routing). Forwarded
  // to invokeOperationRaw.
  execute?: ExecuteFn;
}

// Start the bridge server. Returns once it's listening. The caller
// owns the lifetime — call `.close()` at shutdown.
export async function startBridge(
  opts: StartBridgeOpts,
): Promise<BridgeServer> {
  const addr =
    opts.address ??
    defaultBridgeAddress({ sessionCacheDir: opts.sandbox.sessionCacheDir() });
  ...
  // under tmpdir is predictable from the session id, so this is
  // hardening against same-host availability attacks. Windows TCP
  // doesn't need this branch at all.
  if (addr.listen.path) {
...
function handleConnection(
  ...
  socket.on("data", (chunk: string) => {
    ...
    while ((nl = buffer.indexOf("\n")) >= 0) {
      ...
      void dispatchLine(line, socket, opts);
    }
  });
...
async function dispatchLine(
  ...
  try {
    ...
    // Surface it with a synthetic id so the client sees *something*
...
// summary. Exported for testing and for direct mode reuse.
export async function invokeAndSandbox(
  opts: Pick<
    StartBridgeOpts,
    "manifest" | "client" | "sandbox" | "trimRegistry" | "disabledActions" | "execute"
  >,
  operation: string,
  ...
  const summarize =
    op.trim && op.trim in opts.trimRegistry
      ? opts.trimRegistry[op.trim]
      : (x: unknown) => x;

  return opts.sandbox.sandbox(response, {
